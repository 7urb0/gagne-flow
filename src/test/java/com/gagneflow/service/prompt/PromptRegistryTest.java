...
class PromptRegistryTest {
    ...
    class GetContentTests {
        ...
        void activeVersionExists_shouldReturnContent() {
            PromptVersion pv = new PromptVersion("addie_review", 2,
                    "你是一个教育AI助手，请根据输入生成教案。", "修改评分标准");
            pv.setActive(true);
            when(repo.findByPromptNameAndActiveTrue("addie_review"))
                    .thenReturn(Optional.of(pv));

            String content = registry.getContent("addie_review");
        ...
        void noActiveVersion_shouldFallbackToFileLoader() {
            ...
            when(fileLoader.load(eq("addie/addie_review")))
        ...
        void specificVersion_shouldReturnThatVersion() {
            PromptVersion v1 = new PromptVersion("addie_review", 1,
                    "v1 content", "初始版本");
            when(repo.findByPromptNameAndVersionNumber("addie_review", 1))
                    .thenReturn(Optional.of(v1));

            String content = registry.getContent("addie_review", 1);
    ...
    class ActivateTests {
        ...
        void activate_differentVersion_shouldSwitch() {
            PromptVersion oldActive = new PromptVersion("addie_review", 1,
                    "old content", "v1");
            oldActive.setActive(true);

            PromptVersion target = new PromptVersion("addie_review", 2,
                    "new content", "v2");
            target.setActive(false);

            when(repo.findByPromptNameAndActiveTrue("addie_review"))
                    .thenReturn(Optional.of(oldActive));
            when(repo.findByPromptNameAndVersionNumber("addie_review", 2))
                    .thenReturn(Optional.of(target));

            PromptVersion result = registry.activate("addie_review", 2);
        ...
        void activate_nonexistentVersion_shouldThrow() {
            when(repo.findByPromptNameAndVersionNumber("addie_review", 99))
                    .thenReturn(Optional.empty());

            assertThrows(IllegalArgumentException.class,
                    () -> registry.activate("addie_review", 99));
        }
    }
    ...
    class GetActiveVersionTests {
        ...
        void activeExists_shouldReturnVersionNumber() {
            PromptVersion pv = new PromptVersion("addie_analysis", 3, "content", "v3");
            pv.setActive(true);
            when(repo.findByPromptNameAndActiveTrue("addie_analysis"))
                    .thenReturn(Optional.of(pv));

            assertEquals(3, registry.getActiveVersionNumber("addie_analysis"));
        }
        ...
        void noActiveVersion_shouldReturnOne() {
            ...
            assertEquals(1, registry.getActiveVersionNumber("addie_review"));
        }
    }
    ...
    class ListVersionsTests {
        ...
        void shouldReturnAllVersions() {
            PromptVersion v1 = new PromptVersion("addie_review", 1, "c1", "v1");
            PromptVersion v2 = new PromptVersion("addie_review", 2, "c2", "v2");
            when(repo.findByPromptNameOrderByVersionNumberDesc("addie_review"))
                    .thenReturn(List.of(v2, v1));

            List<PromptVersion> versions = registry.listVersions("addie_review");
