...
# Shell 检测: Windows 下可能需要 Git Bash (安装: choco install make)
# 如果 make 找不到 sh，请确保 Git Bash 在 PATH 中
SHELL := $(shell if [ -x "$$(command -v bash)" ]; then echo "bash"; else echo "sh"; fi)

# 配置变量（支持环境变量覆盖）
SERVER_URL ?= http://localhost:9900
UPLOAD_API = $(SERVER_URL)/api/upload
DOCS_DIR = lesson-plan-docs
...
# 默认目标：显示帮助信息
help:
	@echo "$(GREEN)GagneFlow Makefile$(NC)"
	@echo ""
	@echo "可用命令："
...
init:
	@echo "$(GREEN)开始一键初始化 GagneFlow...$(NC)"
	@echo ""
...
upload:
	...
	@if ls $(DOCS_DIR)/prompts/ >/dev/null 2>&1; then \
		echo "$(RED)❌ 检测到 $(DOCS_DIR)/prompts/ 目录！$(NC)"; \
		echo "$(YELLOW)提示词文件应放在 agent-config/prompts/ 而非 lesson-plan-docs/$(NC)"; \
		echo "$(YELLOW)请执行: mv $(DOCS_DIR)/prompts/ agent-config/prompts/$(NC)"; \
		exit 1; \
	fi
	@if grep -rl "PLAN|EXECUTE|FETCH|REVIEW|FINISH" $(DOCS_DIR)/*.md 2>/dev/null | grep -q .; then \
		echo "$(RED)❌ 检测到 lesson-plan-docs/ 下可能混入了提示词文件！$(NC)"; \
		echo "$(YELLOW)以下文件包含 decision 类型关键字:$(NC)"; \
		grep -rl "PLAN|EXECUTE|FETCH|REVIEW|FINISH" $(DOCS_DIR)/*.md 2>/dev/null; \
		echo "$(YELLOW)提示词文件应放在 agent-config/prompts/ 而非 lesson-plan-docs/$(NC)"; \
		exit 1; \
	fi
	@count=0; \
